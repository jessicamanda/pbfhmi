...
if (isset($_POST['save'])) {
    ...
    if (file_exists($fotoupload)) {
        ...
        $status_plg = ($jmlcepat > 0) ? "Tepat Waktu" : "Pulang Cepat";
        ...
        if ($query->num_rows > 0) {
            echo "
                ...
                document.location.href='index.php?hal=absen-pulang';
                </script>
            ";
        } else {
            // Insert the attendance data into the database
            $con->query("INSERT INTO absensi_pulang
                (tanggal_absen_plg, foto_absen_plg, jam_kerja, jam_pulang, pulang_cepat, status_plg, nama_lengkap, latitude, longitude)
                VALUES ('$tanggal_absen_plg', '$foto', '$jam_kerja', '$jam_plg', '$jmlcepat', '$status_plg', '$nama_lengkap', '$latitude', '$longitude')
            ");

            if (mysqli_affected_rows($con) > 0) {
                echo "
                    <script>
                    document.location.href='index.php?hal=riwayat-absen-pulang';
                    </script>
                ";
            } else {
                echo "
