...
if (isset($_POST['save'])) {
    ...
    if ($con->query($query_insert) && $con->query($query_update)) {
        echo "<script>alert('Data berhasil ditambahkan'); document.location.href='index.php?hal=kedatangan-obat';</script>";
    } else {
...
pembelian.namasuplier, terima.tgl_terima, terima.jumlah_terima, SUM(COALESCE(terima.jumlah_terima, 0)) AS total_terima
FROM pembelian
LEFT JOIN terima ON pembelian.id_pembelian = terima.id_pembelian
GROUP BY pembelian.id_pembelian, pembelian.tgl, pembelian.nama_obat, pembelian.jumlah, pembelian.namasuplier
HAVING (pembelian.jumlah - total_terima) != 0
